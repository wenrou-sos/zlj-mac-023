...
| 扫码签到 | 维保员以本人身份扫码 → GPS 定位签到 → 按**电梯类型（客梯/货梯/扶梯）× 维保周期（半月/季/半年/年）**模板逐项检查 → 电子签名；不可代签；换人扫码需确认接手 |
| 保养记录 | 必检项不可跳过、异常必填说明，可补充自定义项（记录中以"必检/补充"区分）；签到时模板整体快照存档，模板调整不影响历史；异常自动生成急修工单；完成后计划自动顺延 |
